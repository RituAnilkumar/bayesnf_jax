"""
Explainability entry point for TE-split ensemble outputs.
...
Requires one extra config key:
    explain.te_group: "time_encoding"   # or "no_time_encoding"

Usage:
    # TE ensemble mode — point at the per-region TE dir:
...
# ---------------------------------------------------------------------------
# TE-specific: resolve run dirs from top_runs_info.csv
# ---------------------------------------------------------------------------

def _find_run_subdirs_te(ensemble_dir: str, te_group: str, stage: str) -> list[str]:
    """
    Load top_runs_info.csv from {ensemble_dir}/{te_group}/ and return the
    run_dir paths listed there (original multirun dirs with pkl files).
    """
    top_runs_csv = os.path.join(ensemble_dir, te_group, "top_runs_info.csv")
    if not os.path.exists(top_runs_csv):
        raise FileNotFoundError(
            f"top_runs_info.csv not found at {top_runs_csv}\n"
            "Make sure ensemble_uncertainty_time_encoding.py has been run and "
            "ensemble_dir points at the per-region TE output folder "
            "(e.g. outputs/ensemble_te_r2/r01)."
        )
    ...
    for rd in df["run_dir"].values:
        ...
        if not os.path.exists(pkl_path):
            warnings.warn(f"[explain_te] {pkl_name} not found in {rd} — skipping.")
            continue
        run_dirs.append(rd)
    ...
    if not run_dirs:
        raise FileNotFoundError(
            ...
            "Check that the multirun output is still accessible."
        )
    return run_dirs
...
# ---------------------------------------------------------------------------
# Ensemble model weighting (identical to main_explain.py)
# ---------------------------------------------------------------------------
...
def _compute_ensemble_weights(
    ...
    try:
        ...
        warnings.warn(f"[explain_te] Cannot import weighting helpers: {e} — falling back to equal weights.")
        return None

    parents = {os.path.dirname(d) for d in run_dirs}
    if len(parents) != 1:
        warnings.warn("[explain_te] Ensemble members span multiple parents — equal weights used.")
        return None
    ...
    try:
        ...
        warnings.warn(f"[explain_te] build_results_df failed: {e} — equal weights used.")
        return None

    valid = df.dropna(subset=["composite"]).copy().reset_index(drop=True)
    if len(valid) == 0:
        warnings.warn("[explain_te] No runs with complete metrics — equal weights used.")
        return None
    ...
    if missing > 0:
        warnings.warn(
            f"[explain_te] {missing}/{len(run_dirs)} ensemble members have no metrics — "
...
@hydra.main(config_path="conf", config_name="config_explain_te", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    te_group     = ex.get("te_group")

    if not ensemble_dir:
        raise ValueError("explain.ensemble_dir must be set for main_explain_te.py")
    if not te_group:
        raise ValueError(
            "explain.te_group must be set (choose 'time_encoding' or 'no_time_encoding')"
        )

    # Output dir: {output_dir}/{region_tag}/{te_group}
    region_tag = os.path.basename(ensemble_dir.rstrip("/"))
    out_dir = os.path.join(cfg.model.output_dir, region_tag, te_group)
    os.makedirs(out_dir, exist_ok=True)
    print(f"[explain_te] Output dir: {os.path.abspath(out_dir)}")

    # ------------------------------------------------------------------
    # Ensemble mode — resolve run dirs from top_runs_info.csv
    # ------------------------------------------------------------------
    print(f"[explain_te] Mode: ensemble ({ensemble_dir} / {te_group})")
    run_subdirs = _find_run_subdirs_te(ensemble_dir, te_group, stage)
    print(f"[explain_te] Found {len(run_subdirs)} ensemble members in top_runs_info.csv")
    ...
    if len(full_key_counts) > 1:
        print(f"[explain_te] Multiple arch+TE combinations across ensemble members:")
    ...
    for (d, p), fk in zip(all_params_raw, full_keys):
        if fk == majority_full:
            ...
            print(f"[explain_te] Skipping {os.path.basename(d)} — "
                  f"arch/TE {fk} != majority {majority_full}")

    print(f"[explain_te] Using {len(all_params)}/{len(all_params_raw)} compatible members "
    ...
    if ensemble_weights is not None:
        print(f"[explain_te] Weighting='{weighting}', temperature={temperature}")
        print(f"  Weight range: [{ensemble_weights.min():.4f}, {ensemble_weights.max():.4f}]")
    else:
        print(f"[explain_te] Weighting='equal' ({len(all_params)} members)")
    ...
    if top_k_models is not None and top_k_models < len(all_params):
        ...
        print(f"[explain_te] top_k_models={top_k_models}: reduced to {len(all_params)} members")
    ...
    label = f"{stage}_ensemble{len(all_params)}_{te_group}"
    ...
    print(f"[explain_te] Saved {attr_path}  ({len(attr_df)} rows)")
    ...
    if exclude:
        ...
        if keep_idx:
            ...
            print(f"[explain_te] Generating masked plots  (excluding: {dropped})")
            _generate_plots(
                mean_attrs[:, keep_idx],
                std_attrs[:, keep_idx],
