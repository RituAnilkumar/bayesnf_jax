"""
src/hyperparam_tuning_te.py

Variant of hyperparam_tuning.py for sweeps that include
...
plots + ranked tables comparing time-encoding vs no-time-encoding configurations.

Expected multirun layout (one SLURM job per region):
  {multirun_root}/{run_id}/.hydra/overrides.yaml
  {multirun_root}/{run_id}/metrics_glambie_test.csv
  {multirun_root}/{run_id}/._cv/metrics_oos.csv

Metrics used:
  - LOYO RMSE       : pretrain out-of-sample (leave-one-year-out)
  - GLaMBIE RMSE    : finetune vs held-out GLaMBIE years
  - Composite score : weighted average of per-region normalised LOYO + GLaMBIE RMSE

Usage:
  python src/hyperparam_tuning_te.py
  python src/hyperparam_tuning_te.py --config conf/config_hyperparam_tuning.yaml
  python src/hyperparam_tuning_te.py --multirun_root /path/to/multirun/r06_*
...
# ---------------------------------------------------------------------------
# Constants  (use_time_encoding replaces heteroscedastic as the sweep axis)
# ---------------------------------------------------------------------------

SWEEP_PARAMS = [
    "model.model_nlayers",
    "model.model_nhidden",
    "model.use_time_encoding",
]

PARAM_SHORT = {
    "model.model_nlayers":    "nlayers",
    "model.model_nhidden":    "nhidden",
    "model.use_time_encoding": "use_time_encoding",
}

PARAM_LABEL = {
    "nlayers":          "N layers",
    "nhidden":          "Hidden width",
    "use_time_encoding": "Time encoding",
}

HEATMAP_PAIRS = [
    ("nlayers",          "nhidden"),
    ("use_time_encoding", "nlayers"),
    ("use_time_encoding", "nhidden"),
]
...
def build_results_df(
    ...
    Columns: region, run_id, run_dir, nlayers, nhidden, use_time_encoding,
...
def add_composite_score(
    ...
    if all_loyo_only:
        print("  WARNING: glambie_rmse is NaN for all runs. Scoring by loyo_rmse only.")
...
def plot_marginals(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for metric_col, metric_label in metrics:
        ...
        for ax, short in zip(axes, param_cols):
            unique_vals = sorted(valid[short].dropna().unique(), key=_to_num)
            pairs = [
        ...
        fig.tight_layout()
        _savefig(fig, output_dir / "plots" / f"marginals_{metric_col}.png")


def plot_heatmaps(df: pd.DataFrame, output_dir: Path) -> None:
    valid = df.dropna(subset=["composite"])
    regions = sorted(valid["region"].unique())
    n_panels = len(regions) + 1

    for p1, p2 in HEATMAP_PAIRS:
        vals1 = sorted(valid[p1].dropna().unique(), key=_to_num)
        vals2 = sorted(valid[p2].dropna().unique(), key=_to_num)
        ...
        all_data = np.concatenate(
            [g[~np.isnan(g)] for g in all_grids] + [agg_grid[~np.isnan(agg_grid)]]
        )
        if all_data.size == 0:
            plt.close(fig)
        ...
        for ax_idx, (label, grid) in enumerate(
            ...
            ax.set_xticklabels([str(v) for v in vals1], fontsize=7, rotation=30)
            ax.set_yticks(range(len(vals2)))
            ax.set_yticklabels([str(v) for v in vals2], fontsize=7)
        ...
        fig.tight_layout()
        _savefig(fig, output_dir / "plots" / f"heatmap_{p1}_vs_{p2}.png")
...
def plot_parallel_coords(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for idx in range(len(valid)):
        ...
        alpha  = 0.15 + 0.5 * (1 - c_norm(composite[idx]))
        ax.plot(range(len(param_cols)), y_vals, color=color, alpha=float(alpha), linewidth=0.8)

    ax.set_xticks(range(len(param_cols)))
    ax.set_xticklabels([PARAM_LABEL.get(p, p) for p in param_cols], fontsize=10, rotation=15)
...
def plot_nhidden_vs_rmse(df: pd.DataFrame, output_dir: Path) -> None:
    """nhidden on x-axis, RMSE on y-axis, one line per (nlayers, use_time_encoding) combo.

    Solid lines = time encoding on (True); dashed = time encoding off (False).
    """
    ...
    has_ute  = "use_time_encoding" in df.columns and df["use_time_encoding"].notna().any()
    ute_vals = (
        sorted(df["use_time_encoding"].dropna().unique(),
               key=lambda x: int(x) if isinstance(x, bool) else x)
        if has_ute else [None]
    )
    ...
    for ax, (metric_col, metric_label) in zip(axes, metrics):
        ...
        for nl in nlayers_vals:
            for ute in ute_vals:
                if ute is None:
                    ...
                    sub = valid[(valid["nlayers"] == nl) & (valid["use_time_encoding"] == ute)]
                    ls  = linestyles.get(ute, "-")
                    te_tag = "time_enc" if ute else "no_time_enc"
                    lbl = f"{int(nl)}L {te_tag}"
        ...
        legend_title = "N layers × time encoding" if has_ute else "N layers"
        ax.legend(fontsize=8, title=legend_title)
        ax.grid(alpha=0.35)

    subtitle = (
        "(mean ± std across regions; solid=time_enc, dashed=no_time_enc)"
        if has_ute else "(mean ± std across regions)"
    )
...
def plot_te_comparison(df: pd.DataFrame, output_dir: Path) -> None:
    """Box plots comparing time_encoding=True vs time_encoding=False groups
    for each metric, broken down by region.
    """
    if "use_time_encoding" not in df.columns or df["use_time_encoding"].isna().all():
        return
    if df["use_time_encoding"].nunique() < 2:
        print("  Skipping TE comparison — only one group found")
        return
    ...
    group_labels = ["no time enc (False)", "time enc (True)"]

    for metric_col, metric_label in metrics:
        valid = df.dropna(subset=[metric_col, "use_time_encoding"])
        if valid.empty:
            continue
        ...
        fig.suptitle(
            f"{metric_label}: time encoding on vs off\n(per region)",
            fontsize=12,
        )
        ...
        for ax_idx, region in enumerate(regions):
            ...
            grp_false = sub[sub["use_time_encoding"] == False][metric_col].values
            grp_true  = sub[sub["use_time_encoding"] == True][metric_col].values
            groups = [g for g in [grp_false, grp_true] if len(g) > 0]
            lbls   = [
                gl for g, gl in [(grp_false, group_labels[0]), (grp_true, group_labels[1])]
                if len(g) > 0
            ]
            if not groups:
                ax.set_visible(False)
        ...
        fig.tight_layout()
        _savefig(fig, output_dir / "plots" / f"te_comparison_{metric_col}.png")
...
def plot_param_importance(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for ax, (metric_col, metric_label) in zip(axes, metrics):
        ...
        if imp.empty or imp.max() == 0:
            ax.set_visible(False)
            continue
        bar_colors = ["steelblue" if v >= imp.max() * 0.5 else "lightsteelblue"
                      for v in imp.values]
        ax.barh(
...
def plot_regional_ranking(df: pd.DataFrame, output_dir: Path, top_n: int = 10) -> None:
    ...
    def _config_label(row) -> str:
        ...
        if "use_time_encoding" in row.index:
            parts.append("te=" + ("Y" if row.use_time_encoding else "N"))
        return " ".join(parts)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyse bayesnf_jax TE hyperparameter sweep (use_time_encoding axis)."
    )
    parser.add_argument(
        "--config",
    ...
    output_dir     = Path("hyperparam_analysis_te") / multirun_root.name
    ...
    print(f"\n=== bayesnf_jax TE hyperparameter analysis ===")
    ...
    print("  [8/8] Time encoding group comparison")
    plot_te_comparison(df, output_dir)
